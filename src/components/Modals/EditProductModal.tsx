...
interface EditProductModalProps {
  ...
  onUpdateProduct: (updatedProduct: Iproducts) => void;
}
...
const EditProductModal = ({
  ...
  const productColorHexes = useMemo(() => {
    return productToEdit.colors || [];
  }, [productToEdit]);

  useEffect(() => {
    if (isOpen) {
      setProduct(productToEdit);
      setSelectedColors(productColorHexes);
      setSelectedSizes(productToEdit.sizes || []);
      
      const category = ProductCategories.find(
        (cat) => cat.id === productToEdit.category.id
      ) || ProductCategories[0];
      setSelectedCategory(category);
      
      setErrors(defaultErrors);
    }
  }, [isOpen, productToEdit, productColorHexes]);
